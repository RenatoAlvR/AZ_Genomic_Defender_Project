...
# Load DDPM model
model = DenoisingDiffusionPM.load('weights/ddpm_cancer.pt')

# Load real data for stats
real_data = np.load('preprocessing/mix_cancer/data.npy')
gene_means = np.mean(real_data, axis=0)
gene_stds = np.std(real_data, axis=0) + 1e-6
real_lib_sizes = np.sum(real_data, axis=1)

# Load gene names
gene_names = pd.read_csv('preprocessing/mix_cancer/genes.txt', header=None).values.flatten()
...
# Save and visualize
output_dir = Path('data/GSE161529/cancer/gen_data')
